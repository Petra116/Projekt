...
export class AuthService {
  ...
  registerProducer(producer: Producer){
    //HTTP POST request
    const body = new URLSearchParams();
    body.set('email', producer.email);
    body.set('name', producer.name);
    body.set('address', producer.address);
    body.set('phone', producer.phone);
    body.set('password', producer.password);

    const headers = new HttpHeaders({
      'Content-Type': 'application/x-www-form-urlencoded'
    });

    return this.http.post('http://localhost:5000/app/registerProducer', body, {headers: headers})
  }
